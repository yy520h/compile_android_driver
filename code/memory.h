...
bool read_process_memory(pid_t pid, uintptr_t addr, void* user_buffer, size_t size) {
    ...
    while (size > 0) {
        ...
        if (!pa) {
            //printk(KERN_ERR "translate_linear_address失败\n");
            goto none_phy_addr;
        }

        void* va = phys_to_virt(pa); // 将物理地址转换为虚拟地址
        if (!va) {
           // printk(KERN_ERR "phys_to_virt失败\n");
            goto none_phy_addr;
        }
        ...
        if (copy_to_user(user_buffer, buffer, max)) { // 从内核缓冲区复制到用户缓冲区
          //  printk(KERN_ERR "copy_to_user失败\n");
            goto none_phy_addr;
        }
...
bool write_process_memory(pid_t pid, uintptr_t addr, void* user_buffer, size_t size) {
    ...
    while (size > 0) {
        pa = translate_linear_address(mm, addr);
        if (!pa) {
           // printk(KERN_ERR "translate_linear_address 失败\n");
            goto none_phy_addr;
        }

        void* va = phys_to_virt(pa); // 将物理地址转换为虚拟地址
        if (!va) {
            //printk(KERN_ERR "phys_to_virt 失败\n");
            goto none_phy_addr;
        }

        max = min_t(size_t, PAGE_SIZE - (addr & (PAGE_SIZE - 1)), min_t(size_t, size, PAGE_SIZE));
        if (copy_from_user(buffer, user_buffer, max)) { // 从用户缓冲区复制到内核缓冲区
           // printk(KERN_ERR "copy_from_user失败\n");
            goto none_phy_addr;
        }
