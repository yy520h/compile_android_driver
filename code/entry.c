...
{
	static COPY_MEMORY cm;//读写
	static MODULE_BASE mb;//pid 模块基址头
	static char name[0x100] = {0};
	/*static char key[0x100] = {0};//验证
	...
	switch (cmd) {
		...
		case OP_MODULE_BASE:
			...
			break;
		case OP_MODULE_BASE_VM:
			{
				if (copy_from_user(&mb, (void __user*)arg, sizeof(mb)) != 0 
				|| copy_from_user(name, (void __user*)mb.name, sizeof(name)-1) !=0) {
					return -1;
				}
				mb.base = get_module_base_vm(mb.pid, name, mb.vm_flag);
				if (copy_to_user((void __user*)arg, &mb, sizeof(mb)) !=0) {
					return -1;
				}
			}
    	...
    	case OP_MODULE_PID:
			{
				...
				mb.pid = get_process_pid(mb.name);
				if (copy_to_user((void __user*)arg, &mb, sizeof(mb)) !=0) {
					return -1;
				}
			}
