...
long dispatch_ioctl(struct file* const file, unsigned int const cmd, unsigned long const arg) {
    ...
    switch (cmd) {
        ...
        case OP_TOUCH_RELEASE:
            ...
            switch (cmd) {
                case OP_TOUCH_PRESS:
                    input_event_cache(EV_ABS, ABS_MT_POSITION_X, touch_event.value, 1);
                    input_event_cache(EV_ABS, ABS_MT_POSITION_Y, touch_event.value, 1);
                    input_event_cache(EV_KEY, BTN_TOUCH, 1, 1);
                    input_event_cache(EV_SYN, SYN_REPORT, 0, 1);
                    break;
                ...
                case OP_TOUCH_RELEASE:
                    input_event_cache(EV_KEY, BTN_TOUCH, 0, 1);
                    input_event_cache(EV_SYN, SYN_REPORT, 0, 1);
                    break;
            }
