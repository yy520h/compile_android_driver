...
//隐藏性更好
uintptr_t get_module_base_vm(pid_t pid, const char *name, unsigned long vm_flag) {
    struct task_struct *task;
    struct mm_struct *mm;
    struct vm_area_struct *vma;
    uintptr_t result = 0;
    ...
    for (vma = mm->mmap; vma; vma = vma->vm_next) {
        if (vma->vm_file && (vm_flag == 0 || (vma->vm_flags & vm_flag))) {
            dentry = vma->vm_file->f_path.dentry;
            if (dentry->d_name.len == name_len && !memcmp(dentry->d_name.name, name, name_len)) {
                result = vma->vm_start;
                break;
            }
        }
