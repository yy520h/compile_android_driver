...
int input_mt_report_slot_state_cache(unsigned int tool_type, bool active, int lock) {
    ...
    input_event_cache(EV_ABS, ABS_MT_TOOL_TYPE, tool_type, pool, lock);
    return id;
}
...
// 导出符号以便其他模块可以使用这些函数
EXPORT_SYMBOL(init_touch);
EXPORT_SYMBOL(exit_touch);
EXPORT_SYMBOL(get_event_pool);
EXPORT_SYMBOL(input_event_cache);
EXPORT_SYMBOL(get_rand_str);
